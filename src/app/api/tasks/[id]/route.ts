...
export async function GET(request: NextRequest, { params }: Props) {
  try {
    ...
    if (!product) return NextResponse.json({ message: "المهمة الذي تحاول عرض بياناتها غير موجودة" }, { status: 404 });
...
export async function DELETE(request: NextRequest, { params }: Props) {
  try {
    ...
    const id = (await params).id;
    const product = await prisma.task.findUnique({ where: { id }, include: { assignedTo: true } });
    if (!product) return NextResponse.json({ message: "المنتج الذي تحاول حذف بياناته غير موجود" }, { status: 404 });
